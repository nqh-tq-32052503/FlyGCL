...
# bash scripts/run_baselines_hide_adapter.sh [GPU_ID] [SEEDS] [DATASET] [EXTRA_NOTE]
# --------------------------------------------------------------
...
echo "========================================="
echo "Starting HiDe-Adapter Baseline Experiment"
...
# Run HiDe-Adapter experiment
extra=("${@:5}") ; extract_backbone_and_filter_args "${extra[@]}"
BACKBONE_TO_USE="${PARSED_BACKBONE:-${BACKBONE:-vit_base_patch16_224}}"
run_experiment "hide_adapter" "$BACKBONE_TO_USE" "adam" 0.003 "${FILTERED_ARGS[@]}"

echo "========================================="
echo "HiDe-Adapter experiment completed!"
