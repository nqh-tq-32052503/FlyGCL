...
# bash scripts/run_baselines_sdlora.sh [GPU_ID] [SEEDS] [DATASET] [EXTRA_NOTE]
# --------------------------------------------------------------
...
echo "========================================="
echo "Starting SD-LoRA Baseline Experiment"
...
# Run SD-LoRA experiment (follow L2P optimizer/LR settings)
extra=("${@:5}"); extract_backbone_and_filter_args "${extra[@]}"
BACKBONE_TO_USE="${PARSED_BACKBONE:-${BACKBONE:-vit_base_patch16_224}}"
run_experiment "sdlora" "$BACKBONE_TO_USE" "adam" 0.005 "${FILTERED_ARGS[@]}"

echo "========================================="
echo "SD-LoRA experiment completed!"
