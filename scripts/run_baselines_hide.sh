...
# bash scripts/run_baselines_hide.sh [GPU_ID] [SEEDS] [DATASET] [EXTRA_NOTE]
# --------------------------------------------------------------
...
echo "========================================="
echo "Starting HiDe-GCL Baseline Experiment"
...
# Run only HiDe-GCL experiment
extra=("${@:5}") ; extract_backbone_and_filter_args "${extra[@]}"
BACKBONE_TO_USE="${PARSED_BACKBONE:-${BACKBONE:-vit_base_patch16_224}}"
run_experiment "hide" "$BACKBONE_TO_USE" "adam" 0.003 "${FILTERED_ARGS[@]}"

echo "========================================="
echo "HiDe-GCL experiment completed!"
