...
# bash scripts/run_baselines_hide_lora.sh [GPU_ID] [SEEDS] [DATASET] [EXTRA_NOTE]
# --------------------------------------------------------------
...
echo "========================================="
echo "Starting HiDe-LoRA Baseline Experiment"
...
# Run HiDe-LoRA experiment
extra=("${@:5}") ; extract_backbone_and_filter_args "${extra[@]}"
BACKBONE_TO_USE="${PARSED_BACKBONE:-${BACKBONE:-vit_base_patch16_224}}"
run_experiment "hide_lora" "$BACKBONE_TO_USE" "adam" 0.003 "${FILTERED_ARGS[@]}"

echo "========================================="
echo "HiDe-LoRA experiment completed!"
